...
Everything keys off the chat_messages_total counter (see observability.py).
Needs GRAFANA_URL, GRAFANA_API_TOKEN (a glsa_ service-account token, not the OTLP
push auth), GRAFANA_ALERT_EMAIL and GRAFANA_PROM_UID in backend/.env.
...
FOLDER_UID = "document-retrieval-system"
FOLDER_TITLE = "Document Retrieval System"
CONTACT_POINT = "drs-email"
RULE_GROUP = "document-retrieval-system"
MUTE_NAME = "drs-muted"
...
def alert_rule(title, expr, for_dur, no_data, summary, severity, evaluator):
    ...
    return {
        ...
        "noDataState": no_data,
        "execErrState": "Error",
        "labels": {"service": "document-retrieval-system", "severity": severity},
...
RULES = [
    ...
    alert_rule(
        "Document Retrieval System — no messages (30m)",
    ...
    alert_rule(
        "Document Retrieval System — message errors",
...
def ensure_route(tree):
    ...
    if ours is None:
        ours = {"receiver": CONTACT_POINT,
                "object_matchers": [["service", "=", "document-retrieval-system"]],
                "continue": False}
        routes.append(ours)
        changed = True
...
def main():
    ...
    if DRY:
        ...
        if CREATE_ALERTS:
            ...
            print(f"\n### Route: receiver={CONTACT_POINT}, match service=document-retrieval-system"
    ...
    if not CREATE_ALERTS:
        ...
        print("\nDone. Grafana -> Dashboards -> 'Document Retrieval System — Overview'.")
        return
    ...
    print(f"\nDone ({tail}). Grafana -> Dashboards -> 'Document Retrieval System — Overview'; Alerting -> Alert rules.")
